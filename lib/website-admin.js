...
export async function getWebsiteAdminContext() {
  ...
  if (!user) return { supabase, user: null, profile: null, company: null, companyId: null, error: 'UNAUTHENTICATED' };
  ...
  if (
    ...
    return { supabase, user, profile: null, company: null, companyId: null, error: 'NOT_ADMIN' };
  }

  if (!profile.company_id) {
    return { supabase, user, profile, company: null, companyId: null, error: 'NO_COMPANY' };
  }
  ...
  if (companyError || !company) {
    return { supabase, user, profile, company: null, companyId: profile.company_id, error: 'COMPANY_ACCESS' };
  }

  if (company.subscription_status && company.subscription_status !== 'active') {
    return { supabase, user, profile, company, companyId: profile.company_id, error: 'SUBSCRIPTION_INACTIVE' };
  }

  return { supabase, user, profile, company, companyId: profile.company_id, error: null };
}
