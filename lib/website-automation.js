...
const renderTemplate = (value, order = {}) => String(value ?? '')
  .replace(/{{\s*order_number\s*}}/gi, order.order_number || '')
  .replace(/{{\s*customer_name\s*}}/gi, order.customer_name || '')
  .replace(/{{\s*customer_phone\s*}}/gi, order.customer_phone || '')
  .replace(/{{\s*customer_email\s*}}/gi, order.customer_email || '')
  .replace(/{{\s*order_status\s*}}/gi, order.order_status || '')
  .replace(/{{\s*tracking_url\s*}}/gi, order.tracking_token ? `/orders/${order.id}?token=${encodeURIComponent(order.tracking_token)}` : '');
  ...
  for (const automation of automations || []) {
    ...
    try {
      if (automation.action_type === 'create_notification') {
        ...
        const subject = renderTemplate(String(config.subject || `${automation.name}${order?.order_number ? ` · ${order.order_number}` : ''}`), order).slice(0, 250);
        const template = renderTemplate(String(config.message || config.body || `Your order ${order?.order_number || ''} was updated to ${order?.order_status || normalizedTrigger}.`), order).slice(0, 5000);
