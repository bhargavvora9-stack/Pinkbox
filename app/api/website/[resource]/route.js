...
export async function GET(request,{params}){
  ...
  if(resource==='inventory'){
    const [products,tx]=await Promise.all([
      ...
      supabase.from('website_inventory_transactions').select('id,product_id,change_quantity,quantity_before,quantity_after,reason,created_at').eq('company_id',companyId).order('created_at',{ascending:false}).limit(100)
  ...
  if(resource==='settings')query=query.maybeSingle(); else if(['products','orders'].includes(resource))query=query.order('created_at',{ascending:false}).limit(SAFE_LIMIT); else if(resource==='images')query=query.order('sort_order').limit(SAFE_LIMIT); else query=query.order('sort_order',{ascending:true}).order('created_at',{ascending:false}).limit(SAFE_LIMIT);
...
export async function POST(request,{params}){
  ...
  if(resource==='inventory'){
    ...
    const {data:tx,error:tErr}=await supabase.from('website_inventory_transactions').insert({company_id:companyId,product_id:productId,change_quantity:change,quantity_before:before,quantity_after:after,reason:cleanString(body.reason,500)||'Manual adjustment',created_by:user.id}).select().single(); if(tErr)return jsonError(tErr.message,500);
  ...
  const payload=sanitizePayload(resource,body,companyId); if(resource==='orders')payload.order_number=payload.order_number||`24C-${Date.now().toString().slice(-9)}`; if(resource==='products'&&!payload.sku)payload.sku=`24C-${Date.now().toString().slice(-8)}`; if(resource==='products'&&!payload.slug)payload.slug=slugify(payload.title); if(resource==='categories'&&!payload.slug)payload.slug=slugify(payload.name); if(resource==='pages'&&!payload.slug)payload.slug=slugify(payload.title); if(resource==='discounts'&&payload.code)payload.code=payload.code.toUpperCase();
...
export async function PATCH(request,{params}){
  const {resource}=await params,ctx=await getWebsiteAdminContext(); if(ctx.error)return jsonError(ctx.error==='UNAUTHENTICATED'?'Please login.':'Access denied.',ctx.error==='UNAUTHENTICATED'?401:403); const {supabase,companyId,user}=ctx; const body=await request.json().catch(()=>null); if(!body?.id)return jsonError('Record id is required.'); if(resource==='customers')return jsonError('Customer master is managed from BK CRM.',405); const table=tableFor(resource); if(!table||resource==='settings')return jsonError('Invalid update resource.',405); const {data:oldData}=await supabase.from(table).select('*').eq('company_id',companyId).eq('id',body.id).maybeSingle(); if(!oldData)return jsonError('Record not found.',404); const payload=sanitizePayload(resource,body,companyId,true); delete payload.id; delete payload.company_id; delete payload.created_at; payload.updated_at=new Date().toISOString(); const {data,error:dbError}=await supabase.from(table).update(payload).eq('company_id',companyId).eq('id',body.id).select().single(); if(dbError)return jsonError(dbError.message,500); if(resource==='orders'&&body.order_status&&body.order_status!==oldData.order_status)await supabase.from('website_order_status_history').insert({company_id:companyId,order_id:data.id,status:data.order_status,note:cleanString(body.status_note,500)||null,created_by:user.id}); await audit(supabase,{companyId,userId:user.id,action:`${resource}.update`,entityType:table,entityId:data.id,oldData,newData:data}); return Response.json({data});
}
...
function sanitizePayload(resource,b,companyId,partial=false){const payload={company_id:companyId}; const text=(k,max=5000)=>{if(!partial||b[k]!==undefined)payload[k]=cleanString(b[k],max)||null}; const num=(k,f=0)=>{if(!partial||b[k]!==undefined)payload[k]=Number.isFinite(Number(b[k]))?Number(b[k]):f}; const bool=(k,f=false)=>{if(!partial||b[k]!==undefined)payload[k]=b[k]===true||b[k]==='true'};
  if(resource==='categories'){text('name',120);text('slug',180);text('description',2000);text('image_url',1000);text('parent_id',100);num('sort_order');bool('is_active',true);text('seo_title',180);text('seo_description',320)}
  if(resource==='products'){text('sku',100);text('title',180);text('slug',180);text('short_description',1000);text('description',10000);text('brand',120);num('price');num('compare_at_price');num('cost_price');num('gst_percent');text('hsn_code',30);text('barcode',100);num('weight');num('stock_quantity');num('low_stock_threshold');bool('track_inventory',true);bool('allow_backorder',false);bool('featured',false);bool('is_active',true);text('seo_title',180);text('seo_description',320);text('seo_keywords',1000);text('crm_product_id',100)}
  ...
  if(resource==='orders'){text('order_number',80);text('customer_id',100);text('customer_name',180);text('phone',40);text('email',160);payload.shipping_address=b.shipping_address&&typeof b.shipping_address==='object'?b.shipping_address:{};payload.billing_address=b.billing_address&&typeof b.billing_address==='object'?b.billing_address:{};num('subtotal');num('discount_amount');num('shipping_amount');num('tax_amount');num('total_amount');text('payment_method',50);text('payment_status',40);text('order_status',40);text('notes',3000)}
  return payload;
}
