...
function sanitizePayload(resource,b,companyId,partial=false){const payload={company_id:companyId}; const text=(k,max=5000)=>{if(!partial||b[k]!==undefined)payload[k]=cleanString(b[k],max)||null}; const num=(k,f=0)=>{if(!partial||b[k]!==undefined)payload[k]=Number.isFinite(Number(b[k]))?Number(b[k]):f}; const bool=(k,f=false)=>{if(!partial||b[k]!==undefined)payload[k]=b[k]===true||b[k]==='true'};
  ...
  if(resource==='products'){text('sku',100);text('title',180);text('slug',180);text('short_description',1000);text('description',10000);text('brand',120);num('price');num('compare_at_price');num('cost_price');num('gst_percent');text('hsn_code',30);text('barcode',100);num('weight');num('stock_quantity');num('low_stock_threshold');bool('track_inventory',true);bool('allow_backorder',false);bool('featured',false);bool('is_active',true);if(!partial||b.cod_override!==undefined)payload.cod_override=b.cod_override===null?null:(b.cod_override===true||b.cod_override==='true');if(!partial||b.online_payment_override!==undefined)payload.online_payment_override=b.online_payment_override===null?null:(b.online_payment_override===true||b.online_payment_override==='true');if(!partial||b.shipping_charge_override!==undefined)payload.shipping_charge_override=b.shipping_charge_override===null?null:(Number.isFinite(Number(b.shipping_charge_override))?Number(b.shipping_charge_override):null);text('seo_title',180);text('seo_description',320);text('seo_keywords',1000)}
