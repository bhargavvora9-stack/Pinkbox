...
function sanitizePayload(resource,b,companyId,partial=false){const payload={company_id:companyId}; const text=(k,max=5000)=>{if(!partial||b[k]!==undefined)payload[k]=cleanString(b[k],max)||null}; const num=(k,f=0)=>{if(!partial||b[k]!==undefined)payload[k]=Number.isFinite(Number(b[k]))?Number(b[k]):f}; const bool=(k,f=false)=>{if(!partial||b[k]!==undefined)payload[k]=b[k]===true||b[k]==='true'}; const nbool=(k)=>{if(!partial||b[k]!==undefined)payload[k]=(b[k]===null||b[k]===undefined||b[k]==='')?null:(b[k]===true||b[k]==='true')}; const nnum=(k)=>{if(!partial||b[k]!==undefined)payload[k]=(b[k]===null||b[k]===undefined||b[k]==='')?null:(Number.isFinite(Number(b[k]))?Number(b[k]):null)};
  if(resource==='categories'){text('name',120);text('slug',180);text('description',2000);text('parent_id',100);text('image_url',1000);num('sort_order');bool('is_active',true);text('seo_title',180);text('seo_description',320)}
  if(resource==='products'){text('sku',100);text('title',180);text('slug',180);text('short_description',1000);text('description',10000);text('brand',120);num('price');num('compare_at_price');num('cost_price');num('gst_percent');text('hsn_code',30);text('barcode',100);num('weight');num('stock_quantity');num('low_stock_threshold');bool('track_inventory',true);bool('allow_backorder',false);bool('featured',false);bool('is_active',true);text('seo_title',180);text('seo_description',320);text('seo_keywords',1000);nbool('cod_override');nbool('online_payment_override');nnum('shipping_charge_override')}
  if(resource==='images'){text('product_id',100);text('image_url',1500);text('alt_text',300);num('sort_order');bool('is_primary',false)}
  if(resource==='pages'){text('title',180);text('slug',180);text('image_url',1500);payload.content=b.content&&typeof b.content==='object'?b.content:{html:cleanString(b.content,20000)};text('seo_title',180);text('seo_description',320);bool('is_published',false)}
