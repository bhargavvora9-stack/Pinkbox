...
export async function GET() {
  try {
    ...
    if (!settings) return Response.json({ error: 'Store is not configured.' }, { status: 404 });
    ...
    const [products, categories, banners, sections, pages, menu, theme, footer, blog] = await Promise.all([
      db.from('website_products').select('id,sku,title,slug,short_description,description,brand,price,compare_at_price,gst_percent,stock_quantity,featured,is_active,seo_title,seo_description').eq('company_id', c).eq('is_active', true).order('created_at', { ascending: false }),
    ...
    const activeBanners = (banners.data || []).filter(
      (x) => (!x.starts_at || x.starts_at <= now) && (!x.ends_at || x.ends_at >= now)
    );
    ...
    return Response.json({
      settings: {
        ...
        default_shipping_charge: settings.default_shipping_charge,
...
export async function POST(request) {
  try {
    ...
    if (b.action === 'abandoned_cart') {
      ...
      const payload = {
        company_id: c,
        session_id: String(b.session_id).slice(0, 200),
        customer_name: String(b.name || '').trim() || null,
      ...
      const { data, error } = await db
        ...
        .upsert(payload, { onConflict: 'company_id,session_id' })
        .select('id,status')
        .single();
    ...
    if (!b.name || !b.phone || !b.address || !Array.isArray(b.items) || !b.items.length) {
      return Response.json({ error: 'Name, phone, address and at least one product are required.' }, { status: 400 });
    }
    ...
    const { data, error } = await db.rpc('place_website_order', {
      p_company_id: c,
      p_name: address.name,
      ...
      p_note: String(b.note || '').trim() || null,
      p_coupon_code: String(b.coupon_code || '').trim() || null,
    });
    ...
    if (data?.order_id) {
      await db
        .from('website_abandoned_carts')
        .update({ status: 'recovered', recovered_order_id: data.order_id, updated_at: new Date().toISOString() })
        .eq('company_id', c)
        .eq('session_id', String(b.session_id || ''));
    }
