...
export async function GET(request) {
  try {
    ...
    const [products, categories, banners, sections, pages, menu, theme, footer, blog] = await Promise.all([
      db.from('website_products').select('id,sku,title,slug,short_description,description,brand,price,compare_at_price,gst_percent,stock_quantity,low_stock_threshold,track_inventory,allow_backorder,featured,is_active,seo_title,seo_description').eq('company_id', c).eq('is_active', true).order('created_at', { ascending: false }),
    ...
    const selectedProduct = slug ? (products.data || []).find((p) => p.slug === slug) || null : null;
    return Response.json({
      settings: { website_name: settings.website_name, slug: settings.slug, logo_url: settings.logo_url, favicon_url: settings.favicon_url, whatsapp_number: settings.whatsapp_number, phone: settings.phone, email: settings.email, address: settings.address, gstin: settings.gstin, currency: settings.currency, timezone: settings.timezone, cod_enabled: settings.cod_enabled, online_payment_enabled: settings.online_payment_enabled, shipping_enabled: settings.shipping_enabled, free_shipping_threshold: settings.free_shipping_threshold, default_shipping_charge: settings.default_shipping_charge, meta_title: settings.meta_title, meta_description: settings.meta_description },
      products: products.data || [], categories: categories.data || [], banners: activeBanners, sections: sections.data || [], pages: pages.data || [], menu: menu.data || null, theme: theme.data || null, footer: footer.data || null, blog: blog.data || [], selectedProduct,
    });
  } catch (error) {
    console.error('Storefront GET failed:', error);
    return Response.json({ error: error instanceof Error ? error.message : 'Storefront is temporarily unavailable.' }, { status: 500 });
  }
}
...
export async function POST(request) {
  try {
    ...
    console.error('Storefront POST failed:', error);
    return Response.json({ error: error instanceof Error ? error.message : 'Storefront request failed.' }, { status: 500 });
  }
}
