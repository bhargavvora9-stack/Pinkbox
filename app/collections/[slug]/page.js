...
export async function generateMetadata({params}) {
  ...
  return c ? { title:`${c.category.name} | PinkBox`, description:`Shop ${c.category.name} products from PinkBox with reliable hygiene and everyday protection across India.`, alternates:{canonical:`/collections/${encodeURIComponent(slug)}`}, openGraph:{type:'website',url:`/collections/${encodeURIComponent(slug)}`,title:`${c.category.name} | PinkBox`,description:`Explore the PinkBox ${c.category.name} collection.`,images:c.category.image_url?[c.category.image_url]:undefined} } : {title:'Collection | PinkBox'};
}

export default async function CollectionPage({params}) {
  const {slug}=await params; const c=await getCollection(slug); if(!c) notFound();
  const url=absoluteUrl(`/collections/${encodeURIComponent(c.category.slug)}`);
  const schema={
    '@context':'https://schema.org','@type':'CollectionPage',name:c.category.name,description:`Explore the PinkBox ${c.category.name} collection.`,url,
    mainEntity:{'@type':'ItemList',numberOfItems:c.products.length,itemListElement:c.products.slice(0,50).map((p,i)=>({'@type':'ListItem',position:i+1,name:p.title,url:absoluteUrl(`/products/${encodeURIComponent(p.slug)}`)}))}
  };
  const crumbs={'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[
    {'@type':'ListItem',position:1,name:'Home',item:absoluteUrl('/')},{'@type':'ListItem',position:2,name:'Products',item:absoluteUrl('/products')},{'@type':'ListItem',position:3,name:c.category.name,item:url}
  ]};
  return <main className="min-h-screen bg-white text-[#171717]">
    <script type="application/ld+json">{JSON.stringify(schema)}</script><script type="application/ld+json">{JSON.stringify(crumbs)}</script>
