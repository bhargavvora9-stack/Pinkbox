-- PinkBox checkout integrity hardening
-- Replaces the deployed order-placement RPC with aggregate stock checks,
-- integer quantity bounds, input validation and payment-method validation.

create or replace function public.place_website_order(p_company_id uuid, p_name text, p_phone text, p_email text, p_address jsonb, p_items jsonb, p_payment_method text default 'COD'::text, p_note text default null::text, p_coupon_code text default null::text) returns jsonb language plpgsql security definer set search_path to 'public' as $function$
declare
  v_order_id uuid; v_customer_id uuid; v_order_no text; v_subtotal numeric:=0; v_shipping numeric:=0; v_discount numeric:=0; v_total numeric:=0; v_item jsonb; v_product public.website_products%rowtype; v_qty numeric; v_line numeric; v_before numeric; v_after numeric; v_coupon public.website_discounts%rowtype; v_code text:=upper(trim(coalesce(p_coupon_code,''))); v_agg_qty numeric;
begin
  ...
  if upper(coalesce(p_payment_method,'COD')) not in ('COD','ONLINE') then raise exception 'Invalid payment method.'; end if;
  ...
  select case when not coalesce(shipping_enabled,true) then 0 when coalesce(free_shipping_threshold,0)>0 and (v_subtotal-v_discount)>=free_shipping_threshold then 0 else coalesce(default_shipping_charge,0) end into v_shipping from public.website_settings where company_id=p_company_id and slug='pinkbox';
